  ...
  const handleDayPickerSelect = async (date: Date | undefined) => {
    if (!date) {
      ...
      if (date.getDay() === 0) {
        ...
        setIsLoading(true)
        try {
          const availableSlots = await getAvailableSlots(format(date, "yyyyMMdd"))
  ...
  const validateStep = (step: number) => {
    ...
    switch (step) {
      ...
      case 2:
        ...
        break
      case 3:
        if (!nome) newErrors.nome = "Inserire nome"
        if (!cognome) newErrors.cognome = "Inserire cognome"
        if (!email) newErrors.email = "Inserire email"
        else if (!/\S+@\S+\.\S+/.test(email)) newErrors.email = "Email non valida"
        if (!telefono) newErrors.telefono = "Inserire numero di telefono"
        else if (!/^\+?[0-9]{10,15}$/.test(telefono)) newErrors.telefono = "Il numero di telefono non è valido"
        break
    }
